...
class SharedStore {
    ...
    getDefaultCategory() {
        this.getDefaultCategory$ = http.get('/admin/api/options?type=defaultCategory');
        this.getDefaultCategory$.subscribe(res => {
            if (res.errno === 0) {
    ...
    getTagList() {
        ...
        http.get<Tag[]>('/admin/api/tag').subscribe(
            res => {
                if (res.errno === 0) {
    ...
    getTemplateList(theme: string) {
        http.get('/admin/api/theme?type=templateList', { theme }).subscribe(res => {
            if (res.errno === 0) {
            ...
            .pipe(
                map(res => {
                    res.data.map((item, key) => {
                        ...
                        item.statusText = tools.getStatusText(
                            item.status,
                            item.create_time,
                        );
                    });
